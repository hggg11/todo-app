...
function App() {
  ...
  const matchesFilter = (todo: Todo): boolean => {
  ...
  const activeTodos = todos
    .filter(todo => todo.status === 'ACTIVE' && matchesFilter(todo))
    .sort((a, b) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0));
  const completedTodos = todos
    .filter(todo => todo.status === 'COMPLETED' && matchesFilter(todo))
    .sort((a, b) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0));
  const cancelledTodos = todos
    .filter(todo => todo.status === 'CANCELLED' && matchesFilter(todo))
    .sort((a, b) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0));
  useEffect(() => {
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition ${activeTab === 'todos'
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition ${activeTab === 'calendar'
              ? 'bg-blue-600 text-white'
              : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
              }`}
          >
                  ...
                  <div className="flex gap-1 flex-wrap">
                    {ICONS.map(icon => (
                      <button
                        ...
                        className={`text-xl p-1 rounded transition ${newIcon === icon
                ...
                <div className="flex gap-1">
                  {(['ALL', 'HIGH', 'MEDIUM', 'LOW'] as const).map(p => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-full text-sm border transition ${filterPriority === p
                ...
                <div className="flex gap-1">
                  {(['ALL', 'TODAY', 'THIS_WEEK', 'OVERDUE'] as const).map(d => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-full text-sm border transition ${filterDueDate === d
                        ? 'bg-blue-600 text-white border-blue-600'
                        : 'bg-white text-gray-600 border-gray-300 hover:border-blue-400'
                        }`}
                    >
          ...
          <div className="flex gap-1 flex-wrap">
            {ICONS.map(icon => (
              <button
                ...
                className={`text-xl p-1 rounded transition ${editIcon === icon
                  ? 'bg-blue-100 ring-2 ring-blue-400'
                  : 'hover:bg-gray-100'
                  }`}
              >
